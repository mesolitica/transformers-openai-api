...
def _compute_mask_indices(
    ...
    for input_length in input_lengths:
        ...
        spec_aug_mask_idx = np.concatenate([spec_aug_mask_idx, np.ones(
...
class WhisperFlashAttention2(WhisperAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Beware that with flash_attn<2.1, using q_seqlen != k_seqlen (except for
        # the case q_seqlen == 1) produces a wrong mask (top-left).
...
class WhisperSdpaAttention(WhisperAttention):
    # Copied from
    # transformers.models.bart.modeling_bart.BartSdpaAttention.forward with
    # BART->whisper, Bart->Whisper
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            # TODO: Improve this warning with e.g. `model.config._attn_implementation
        ...
        # The tgt_len > 1 is necessary to match with
...
# Copied from transformers.models.mbart.modeling_mbart.MBartEncoderLayer
# with MBart->Whisper, MBART->WHISPER
...
# Copied from transformers.models.mbart.modeling_mbart.MBartDecoderLayer
# with MBart->Whisper, MBART->WHISPER
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.layers = nn.ModuleList([WhisperDecoderLayer(config, layer_idx=i)
...
class WhisperForCausalLM(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        # If the user passed a tuple or `BaseModelOutput` for encoder_outputs, we
        # extract only the hidden states
