...
class T5DenseGatedActDense(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # we also make sure the weights are not in `int8` in case users will force
        # `_keep_in_fp32_modules` to be `None``
        if (
            isinstance(self.wo.weight, torch.Tensor)
            and hidden_states.dtype != self.wo.weight.dtype
            and self.wo.weight.dtype != torch.int8
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            module.q.weight.data.normal_(mean=0.0,
                                         std=factor * ((d_model * key_value_proj_dim) ** -0.5))
            module.k.weight.data.normal_(mean=0.0, std=factor * (d_model**-0.5))
            module.v.weight.data.normal_(mean=0.0, std=factor * (d_model**-0.5))
            module.o.weight.data.normal_(mean=0.0,
...
class T5Stack(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            ...
            " 'block.1': 1, ...}", FutureWarning, )
        # Check validity of device_map
        self.device_map = (get_device_map(len(self.block), range(
...
# Warning message for FutureWarning: head_mask was separated into two
# input args - head_mask, decoder_head_mask
...
class T5Model(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            ...
            " 0, 'encoder.block.1': 1, ...}", FutureWarning, )
        self.device_map = (
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            ...
            " {'encoder.block.0': 0, 'encoder.block.1': 1, ...}", FutureWarning, )
        self.device_map = (
            get_device_map(len(self.encoder.block), range(torch.cuda.device_count()))
            if device_map is None
            else device_map
...
class T5ForSequenceClassification(T5PreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(T5_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Seq2SeqSequenceClassifierOutput,
...
class T5ForQuestionAnswering(T5PreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(T5_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Seq2SeqQuestionAnsweringModelOutput,
                               config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
